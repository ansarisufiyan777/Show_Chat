package info.infiniteloops.discuss.chat.service;
...
public class ServiceUtils {
    ...
    public static void updateUserStatus(Context context){
        if(isNetworkConnected(context)) {
            ...
            if (!uid.equals("")) {
                Utilities.getFirebaseDBReference().child("user/" + uid + "/status/isOnline").setValue(true);
                Utilities.getFirebaseDBReference().child("user/" + uid + "/status/timestamp").setValue(System.currentTimeMillis());
            }
        }
    }
    ...
    public static void updateFriendStatus(Context context, ListFriend listFriend){
        if(isNetworkConnected(context)) {
            for (Friend friend : listFriend.getListFriend()) {
                ...
                Utilities.getFirebaseDBReference().child("user/" + fid + "/status").addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        if (dataSnapshot.getValue() != null) {
                            ...
                            if ((boolean) mapStatus.get("isOnline") && (System.currentTimeMillis() - (long) mapStatus.get("timestamp")) > StaticConfig.TIME_TO_OFFLINE) {
                                Utilities.getFirebaseDBReference().child("user/" + fid + "/status/isOnline").setValue(false);
                            }
                        }
                    }
